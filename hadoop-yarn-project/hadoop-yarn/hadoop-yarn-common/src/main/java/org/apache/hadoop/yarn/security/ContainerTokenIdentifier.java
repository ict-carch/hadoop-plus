...
public class ContainerTokenIdentifier extends TokenIdentifier {
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    int gCores = in.readInt();
    if(gCores>0)   //lism: add gpu id information
    {
        List<Integer> gpuId = new ArrayList<Integer>();
    	for(int i=0; i<gCores;i++)
    		{
    			gpuId.add(in.readInt());
    		}
    	this.resource = Resource.newInstance(memory, vCores, gCores, gpuId);
    }
    else 
    	this.resource = Resource.newInstance(memory, vCores, gCores, null);
