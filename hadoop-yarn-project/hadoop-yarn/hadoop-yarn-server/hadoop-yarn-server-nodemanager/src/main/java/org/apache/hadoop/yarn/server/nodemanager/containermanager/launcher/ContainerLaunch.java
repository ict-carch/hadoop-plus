...
public class ContainerLaunch implements Callable<Integer> {
  ...
  public void sanitizeEnv(Map<String, String> environment, Path pwd,
     ...
     */
	   ...
	   * @author Tim and Lism
	   * @date 2013-10-16
	   * @description Put GPU number into child JVM's env
	   */
	environment.put(Environment.GPU.name(),String.valueOf(container.getResource().getGPUCores()));
	
	if(container.getResource().getGPUCores()!=0)
	{
		int gpu_id=container.getResource().getGPUId().get(container.getResource().getGPUId().size()-1);
		environment.put(Environment.GPU_ID.name(), String.valueOf(gpu_id));
	}
	else
		environment.put(Environment.GPU_ID.name(),"-1");
