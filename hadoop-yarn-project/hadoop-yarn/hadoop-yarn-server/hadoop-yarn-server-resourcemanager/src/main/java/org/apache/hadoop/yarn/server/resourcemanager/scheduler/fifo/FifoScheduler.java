...
public class FifoScheduler implements ResourceScheduler, Configurable {
  ...
  /*@author: Shengmei Li
   * add the vcore and gpu while reinitialize
   */
  @Override
  ...
  private int assignContainer(FiCaSchedulerNode node, FiCaSchedulerApp application, 
    ...
    int availableContainers_mem = 
      node.getAvailableResource().getMemory() / capability.getMemory(); // TODO: A buggy
                                                                        ...
                                                                        // crash the
                                                                         // scheduler.
    //add by lism: add the cpu limitations
    int avilableContainers_cpu =
    		node.getAvailableResource().getVirtualCores() / capability.getVirtualCores();
    
    int availableContainers = 
    		Math.min(availableContainers_mem, avilableContainers_cpu);
    ...
    if (assignedContainers > 0) {
      for (int i=0; i < assignedContainers; ++i) {
        ...
        /*
         * lism: set the allocated gpu id, here we use only one gpu. 2013-12
         */
       ...
       // LOG.info("[*GPU DEBUG] assign container: <"+ capability.getMemory()+","+capability.getVirtualCores()+","+capability.getGPUCores()+","+capability.getGPUId()+">");
        ...
        //LOG.info("[*GPU DEBUG] allocate container is:" + container.getId() +":" +container.getResource());
        // Allocate!
        ...
        LOG.info("[*GPU DEBUG] request is:" + request.getCapability().toString());
        LOG.info("[*GPU DEBUG] allocate container is:" + container.getId() +":" +container.getResource());
