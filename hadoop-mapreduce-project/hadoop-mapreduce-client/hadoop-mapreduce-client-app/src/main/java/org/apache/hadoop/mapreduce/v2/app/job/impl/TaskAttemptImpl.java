...
public abstract class TaskAttemptImpl implements
  ...
  private int getCpuRequired(Configuration conf, TaskType taskType) {
    ...
    LOG.debug("Task attempt got " + vcores + " CPU.");
    return vcores;
  }
  /*
  * @author Tim adn Lism
  * @date 2013-10-16
  *	@description Get GPU prefer and requirements from configuration file
  */
  private int getGpuRequired(Configuration conf, TaskType taskType)
  {
	  ...
	  if (taskType == TaskType.MAP)
	  {
		  LOG.debug("[GPU DEBUG] MAP TASK");
		  prefer = 
			conf.getBoolean(MRJobConfig.MAP_GPU_PREFER, MRJobConfig.DEFAULT_MAP_GPU_PREFER);
		  if (prefer)
			  gcores = 
			  conf.getInt(MRJobConfig.MAP_GPU_CORES, MRJobConfig.DEFAULT_MAP_GPU_CORES);
	  }
	  else 
		  if (taskType == TaskType.REDUCE)
	  {
		  LOG.debug("[GPU DEBUG] REDUCE TASK");
		  prefer = 
			conf.getBoolean(MRJobConfig.REDUCE_GPU_PREFER, MRJobConfig.DEFAULT_REDUCE_GPU_PREFER);
		  if (prefer)
			  gcores = 
			  conf.getInt(MRJobConfig.REDUCE_GPU_CORES, MRJobConfig.DEFAULT_REDUCE_GPU_CORES);
	  }
	  if(prefer)
		  LOG.debug("[GPU DEBUG] Prefer GPU");
	  else
		  LOG.debug("[GPU DEBUG] NOT Prefer GPU");
//	  LOG.debug("MAP_GPU_PREFER: "+ MRJobConfig.DEFAULT_MAP_GPU_PREFER);
//	  LOG.debug("MAP_GPU_CORES: "+ MRJobConfig.DEFAULT_MAP_GPU_CORES);
//	  LOG.debug("REDUCE_GPU_PREFER: "+ MRJobConfig.DEFAULT_REDUCE_GPU_PREFER);
//	  LOG.debug("REDUCE_GPU_CORES: "+ MRJobConfig.DEFAULT_REDUCE_GPU_CORES);
	  LOG.debug("[GPU DEBUG] This task requires " + gcores + " GPU");
	  return gcores;
  }
